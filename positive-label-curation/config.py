...
METADATA_DIR = os.path.join(DATASET_ROOT, "metadata")  # All CSVs and reports
...
STAGE8_QA_CSV = os.path.join(METADATA_DIR, "qa_corrections.csv")  # QA corrections CSV
STAGE8_STATE_FILE = ".stage8_state.json"  # State persistence file (local to script dir)

# Stage9: Apply QA Corrections (batch apply corrections from Stage8)
STAGE9_QA_CSV = os.path.join(METADATA_DIR, "qa_corrections.csv")  # Input: QA corrections from Stage8
STAGE9_OUTPUT_CSV = os.path.join(METADATA_DIR, "Stage9out_corrections_applied.csv")  # Output: action log
STAGE9_REPORT_TXT = os.path.join(METADATA_DIR, "Stage9_report.txt")  # Output: summary report
